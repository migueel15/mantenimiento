...
/**
 * A binary search tree is a binary tree in which each node has a value and two
 * subtrees, and for each node
 * all elements in the left subtree are less than the node value, and all
...
public interface BinarySearchTreeStructure<T> {
   ...
   * <p>
   * More formally, returns {@code true} if and only if this binary tree contains
   * at least one
   ...
   * 
   * @throws BinaryTreeException if the element is not present in the binary
   *                             search tree
   */
  ...
  /**
   * Returns the maximum depth of this binary tree. The depth of a binary tree is
  ...
  /**
   * Removes the first occurrence of the specified element from this binary search
