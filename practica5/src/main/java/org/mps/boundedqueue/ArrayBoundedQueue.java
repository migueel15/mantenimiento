...
public class ArrayBoundedQueue<T> implements BoundedQueue<T> {
   ...
   * <p>
   * The elements of the bounded queue are stored in the array {@code buffer} as
   * follows:
   * </p>
   * <ul>
   * <li>elements are stored in a circular slice of {@code size} positions that
   * ranges from {@code first} to
   * {@code nextFree-1}, where the next position is computed using modular
   * arithmetic. Elements cannot
   * be {@code null}.
   * </li>
   * <li>the {@code size - buffer.length} available positions are those in a
   * circular slice that ranges from
   * {@code nextFree} to {@code first-1}, once again, using modular arithmetic.
   * Available positions
   * must be {@code null}.
   * </li>
   * </ul>
  ...
  private int first; // index of the position in buffer that stores the first element in the bounded
                     // queue
